...
while [[ $# -gt 0 ]];do
	case $1 in
		-url|--url)	url_template=$2 ;shift;shift;;
		-min|--min_int)	min_int=$2 ;shift;shift;;
		-max|--max_int)	max_int=$2 ;shift;shift;;
		-num|--is_numbering)	is_numbering=$2 ;shift;shift;;	#use name from counter / strg file
		-log|--is_logging)	is_logging=$2 ;shift;shift;;
		-strg|--strg_file)	strg_file=$(realpath "$2") ;shift;shift;;
		-cookie|--cookie_file)	cookie_file=$(realpath "$2") ;shift;shift;;
